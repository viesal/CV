...
img1 = cv.LoadImage("1.jpeg")
img2 = cv.LoadImage("2.jpeg")
...
g = cv.CreateImage(cv.GetSize(img1), cv.IPL_DEPTH_8U, 1)
b = cv.CreateImage(cv.GetSize(img1), cv.IPL_DEPTH_8U, 1)
diff = cv.CreateImage(cv.GetSize(img1), cv.IPL_DEPTH_8U, 3)
smoth = cv.CreateImage(cv.GetSize(img1), cv.IPL_DEPTH_8U, 1)
hsv = cv.CreateImage(cv.GetSize(img1), cv.IPL_DEPTH_8U, 3)
diff2 = cv.CreateImage(cv.GetSize(img1), cv.IPL_DEPTH_8U, 3)
...
path = u"img/sub{}.jpg".format(datetime.strftime(datetime.now(), "%S%f "))  # Уникальное имя для каждого кадра
